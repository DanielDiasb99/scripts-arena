...
def apply_script(protocol, connection, config):
    class ArenaConnection(connection):
	    ...
	    fax,fay,faz = HIDE_COORD[0],HIDE_COORD[1],HIDE_COORD[2]
	    flag = self.team.flag
            if not self.protocol.arena_running:
                ...
                if min_distance > self.protocol.arena_max_spawn_distance:
                    ...
                    self.refill()
		self.protocol.green_team.flag.set(fax,fay,faz)
		self.protocol.blue_team.flag.set(fax,fay,faz)
		flag.update()
		
            else:
		flag = self.team.flag
		green_count = get_team_alive_count(self.protocol.green_team)
		blue_count = get_team_alive_count(self.protocol.blue_team)
		if green_count == 1 and blue_count == 1:
		    if self.world_object.dead == False and self.team == self.protocol.blue_team:
		        flag_x = self.world_object.position.x
			flag_y = self.world_object.position.y
			flag_z = -500
			self.protocol.blue_team.flag.set(flag_x,flag_y,flag_z)
		    if self.world_object.dead == False and self.team == self.protocol.green_team:
		        flag_x1 = self.world_object.position.x
			flag_y1 = self.world_object.position.y
			flag_z1 = -500
			self.protocol.green_team.flag.set(flag_x1,flag_y1,flag_z1)	
		    flag.update()
